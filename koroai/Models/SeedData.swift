...
enum SeedData {
    ...
    static func previewItems(now: Date = .now, calendar: Calendar = .current) -> [FoodItem] {
        ...
        // catId はセクション（FoodCategory）id。旧 chicken/leafy は meat/veg セクションへ統合済み。
        return [
            item("fish",  name: "刺身（まぐろ）", daysLeft: 0,  amount: 0.5),
            item("meat",  name: "鶏むね肉",       daysLeft: 1,  amount: 0.72),
            item("veg",   name: "ほうれん草",     daysLeft: 2,  amount: 0.3),
            item("tofu",  name: "絹ごし豆腐",     daysLeft: 3,  quantity: 1, quantityTotal: 3),
            item("dairy", name: "牛乳",           daysLeft: 4,  amount: 0.72),
            // veg のセクション既定は amount だが、個数指定の意図なので count に明示上書き。
